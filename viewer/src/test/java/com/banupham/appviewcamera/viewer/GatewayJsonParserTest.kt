...
class GatewayJsonParserTest {
    ...
    fun parsesCameraConfigurationWithoutCredentials() {
        val cameras = GatewayJsonParser.cameras(
            """[{"id":"camera01","name":"Cửa trước","host":"192.168.1.7","port":554,"username":"admin","main_path":"Streaming/Channels/101","sub_path":"Streaming/Channels/102","relay_path":"camera01","preview_relay_path":"camera01_sub","enabled":true,"record_enabled":true,"motion_enabled":false}]"""
        )
        assertEquals(1, cameras.size)
        assertEquals("192.168.1.7", cameras.single().host)
        assertEquals("camera01", cameras.single().relayPath)
        assertEquals("camera01_sub", cameras.single().previewRelayPath)
    }
